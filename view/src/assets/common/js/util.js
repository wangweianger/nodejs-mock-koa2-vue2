...
class util {
	...
	ajax(json) {
		...
		let user = this.getStorage('local', 'userMsg') ? JSON.parse(this.getStorage('local', 'userMsg')) : {}
		...
		return $.ajax({
			...
			datatype: "json",
			headers: {
				"token": $.cookie('zane:suid') + '-' + userName
			},
			async: asyncVal,
	...
	*/
	...
	toJson(json) {
		...
		copyJson = copyJson.replace(/['‘“’”]/g, '"')
		// 替换value值中的双引号
		copyJson = copyJson.replace(/\b"(?=([\w\s-_\d\.\*\u4E00-\u9FA5\uf900-\ufa2d]+?))/g, '’')
		// 替换不正常的 , 号
		copyJson = copyJson.replace(/[，]/g, ',')
		// 替换 undefined 为字符串
		...
		try {
			...
			// 不正常开始替换
			copyJson = copyJson.replace(/\{"?([\u4E00-\u9FA5\uf900-\ufa2d'"\d\w_-]*?)"?:/g,($a,$b)=>{
				return `{"${$b}":`;
			}) 
			copyJson = copyJson.replace(/,"?([\u4E00-\u9FA5\uf900-\ufa2d'"\d\w_-]*?)"?:/g,($a,$b)=>{
				return `,"${$b}":`;
			...
			if (typeof(needJson[key]) == 'object') {
				...
				if (Array.isArray(needJson[key])) {
					if (needJson[key].length) {
						...
						json1.defaultVal = JSON.stringify([]);
						json1.type = 'array'
					}
				} else {
					json1.defaultVal = '';
	...
	getSignData(json = {
		...
		let signwx = this.signwx({
			name: json.name || 'wangwei',
			company: json.company || 'morning-star',
			time: this.time(),
